...
`{t:'twocircles',a:[parts,filled],b:[parts,filled]}` `{t:'grid',rows,cols,opts:{sides:true}}`
...
## Notes
- Progress (name + stars) is saved in localStorage under key `hlc-save`,
  per browser and per domain. Changing world `id`s resets earned stars — keep ids stable.
- Difficulty target: ~5th grade. Raise gradually; when adding harder questions,
  keep 1–2 warm-up questions at the start of each world.
